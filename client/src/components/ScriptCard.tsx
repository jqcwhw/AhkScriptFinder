...
export default function ScriptCard({ script, onDownload, onPreview, onDelete }: ScriptCardProps) {
  return (
    <div className="relative group">
      ...
      <Card className="relative flex flex-col bg-gradient-to-br from-slate-800 to-slate-900 border-2 border-emerald-500/20 group-hover:border-emerald-500/40 transition-all duration-300 shadow-lg shadow-emerald-500/5" data-testid={`card-script-${script.id}`}>
        ...
        <CardHeader className="space-y-3 pb-4 pt-6">
          ...
          <CardTitle className="text-xl font-black text-slate-100 tracking-tight" data-testid={`text-scriptname-${script.id}`}>
            {script.name}
          </CardTitle>
        ...
        <CardContent className="flex-1 space-y-4">
          ...
          {script.downloadCount !== undefined && (
            <div className="flex items-center gap-2 text-xs text-slate-400 bg-slate-800/50 rounded-md px-3 py-2">
              ...
              <span className="font-semibold">{script.downloadCount.toLocaleString()} downloads</span>
            </div>
          )}
        </CardContent>
